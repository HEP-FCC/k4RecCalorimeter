...
public:
   ...
   */
  IDMapIndexer(int detID, size_t detIDBits, std::span<const FieldDesc_t> fields, std::span<const uint64_t> ids,
               size_t sizeHint = 0, std::span<const FieldDesc_t> ignoredFields = {});
 ...
 */
template <unsigned NFIELDS>
IDMapIndexer<NFIELDS>::IDMapIndexer(int detID, size_t detIDBits, std::span<const FieldDesc_t> fields,
                                    std::span<const uint64_t> ids, size_t sizeHint /*= 0*/,
                                    std::span<const FieldDesc_t> ignoredFields /* = {}*/)
    : m_map(
          fields, INVALID, ids, [](size_t i) { return i; }, sizeHint),
      m_detID(detID), m_detIDBits(detIDBits), m_cellIDs(ids) {
 ...
 */
template <unsigned NFIELDS>
inline auto IDMapIndexer<NFIELDS>::index(uint64_t id) const -> index_t {
  if ((id & m_otherFieldsMask) != m_otherFieldsVal)
    return INVALID;
 ...
 */
template <unsigned NFIELDS>
inline std::span<const uint64_t> IDMapIndexer<NFIELDS>::cellIDs() const {
  return m_cellIDs;
}
 ...
 */
template <unsigned NFIELDS>
inline std::span<const int> IDMapIndexer<NFIELDS>::detIDs() const {
 ...
 */
template <unsigned NFIELDS>
inline size_t IDMapIndexer<NFIELDS>::detIDBits() const {
  return m_detIDBits;
}
 ...
 */
template <unsigned NFIELDS>
inline size_t IDMapIndexer<NFIELDS>::byteSize() const {
 ...
 */
template <unsigned NFIELDS>
inline void IDMapIndexer<NFIELDS>::printStats(std::ostream& s) const {
