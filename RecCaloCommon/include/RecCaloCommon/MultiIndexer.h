...
public:
   ...
   */
  MultiIndexer(size_t detIDBits, std::span<const ICaloIndexer* const> indexers, ICaloCellConstantsSvc& constsSvc);
 ...
 * @brief Return the index of an identifier.
 */
inline auto MultiIndexer::index(uint64_t id) const -> index_t {
 ...
 * @brief Return the set of all identifiers that we index.
 */
inline std::span<const uint64_t> MultiIndexer::cellIDs() const { return m_cellIDs; }

/**
 * @brief Return the IDs of the detector(s) that we index.
 */
inline std::span<const int> MultiIndexer::detIDs() const { return m_detIDs; }

/**
 * @brief Number of bits in the cell ID used for the detector ID.
 */
inline size_t MultiIndexer::detIDBits() const { return m_detIDBits; }
