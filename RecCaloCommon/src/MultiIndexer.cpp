...
class DummyIndexer : public ICaloIndexer {
  virtual index_t index(uint64_t /*id*/) const override final { return ICaloIndexer::INVALID; }
  virtual std::span<const uint64_t> cellIDs() const override final { return std::span<const uint64_t>(); }
  virtual std::span<const int> detIDs() const override final { return std::span<const int>(); }
 ...
 * @brief Constructor.
 */
MultiIndexer::MultiIndexer(size_t detIDBits, std::span<const ICaloIndexer* const> indexers,
  ...
  if (detIDBits < 1 || detIDBits > 8) {
    throw MultiIndexerException(std::format("detIDBits {} out of range; should be between 1 and 8", detIDBits));
  }
  ...
  for (const ICaloIndexer* indexer : indexers) {
    ...
    if (detIDs.size() != 1) {
      throw MultiIndexerException(std::format("bad indexer returns detIDs of size {}", detIDs.size()));
    }
    if (detIDs[0] > static_cast<int>(m_detIDMask)) {
      throw MultiIndexerException(std::format("bad indexer returns out-of-range detID {}", detIDs[0]));
    }
  ...
  const std::vector<uint64_t>* cellsptr = constsSvc.getObj<std::vector<uint64_t>>(keyName);
  if (!cellsptr) {
  ...
  if (cellsptr->size() != totcells) {
    throw MultiIndexerException(
        std::format("cells vector size mismatch.  Got {}; expected {}", cellsptr->size(), totcells));
  }
