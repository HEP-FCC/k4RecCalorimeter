...
inline uint32_t rng_seed(uint32_t& seed) {
  seed = (1664525 * seed + 1013904223);
  return seed;
}

inline float randf_seed(uint32_t& seed, float rmax, float rmin = 0) {
  return static_cast<float>(rng_seed(seed)) / static_cast<float>(rngmax) * (rmax - rmin) + rmin;
}

inline int randi_seed(uint32_t& seed, int rmax, int rmin = 0) { return static_cast<int>(randf_seed(seed, rmax, rmin)); }
...
class IDMapLookupB : public IDMAP {
public:
  ...
  IDMapLookupB(mapkey_span ids) : IDMAP(fieldDescs(), INVALID, ids, [](size_t i) { return i; }) {}
};
...
template <class IDMAP>
class IDMapLookupE : public IDMAP {
public:
  constexpr static payload_t INVALID = static_cast<payload_t>(-1);
  using FieldDesc_t = typename IDMAP::FieldDesc_t;
  static std::vector<FieldDesc_t> fieldDescs();
  IDMapLookupE(mapkey_span ids) : IDMAP(fieldDescs(), INVALID, ids, [](size_t i) { return i; }) {}
};
...
auto IDMapLookupE<IDMAP>::fieldDescs() -> std::vector<FieldDesc_t> {
  ...
  fielddescs.emplace_back(std::min(bfe_side->offset(), bfe_wheel->offset()), bfe_side->width() + bfe_wheel->width());
...
private:
  struct Field {
    Field(unsigned pos = 0, unsigned nbits = 1) : m_mask(((1ull << nbits) - 1) << pos), m_shift(pos), m_size(0) {}
...
inline uint32_t ArrLookup::index(mapkey_t k) const {
...
inline payload_t ArrLookup::lookup(mapkey_t k) const { return m_arr[index(k)]; }
...
public:
  ...
  class RunTimer {
  public:
    RunTimer(boost::timer::cpu_timer& timer) : m_timer(&timer) { timer.resume(); }
...
TesterBase::TesterBase(const std::string& name, mapkey_span ids) : m_name(name), m_ids(ids) {}
...
template <class LOOKUP>
Tester<LOOKUP>::Tester(const std::string& name, mapkey_span ids) : TesterBase(name, ids), m_lookup(ids) {
