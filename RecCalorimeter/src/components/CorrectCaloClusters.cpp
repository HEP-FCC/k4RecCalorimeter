...
StatusCode CorrectCaloClusters::execute() {
  ...
  if (m_upstreamCorr) {
    ...
    if (sc.isFailure()) {
      ...
      info() << "Running the upstream correction." << endmsg;
  ...
  if (m_downstreamCorr) {
    ...
    if (sc.isFailure()) {
      ...
      info() << "Running the downstream correction." << endmsg;
    }
  }
  ...
  if (m_benchmarkCorr) { 
    ...
    if (sc.isFailure()) {
      ...
      info() << "Running the benchmark correction." << endmsg;
    }
  }
...
StatusCode CorrectCaloClusters::applyBenchmarkCorr(const edm4hep::ClusterCollection* inClusters,
  ...
  for (size_t j = 0; j < inClusters->size(); ++j) {
    ...
    verbose() << "********************************************************************" << endmsg;
    verbose() << "Cluster energy: " << inClusters->at(j).getEnergy() << endmsg;
    verbose() << "totalEnergyInECal+HCal from hits: " << totalEnergyInECal+totalEnergyInHCal << endmsg;
    verbose() << "********************************************************************" << endmsg;
    verbose() << "Corrected cluster energy benchmark: " << outClusters->at(j).getEnergy() << endmsg;
    verbose() << "********************************************************************" << endmsg;    
    } 
  }            
  return StatusCode::SUCCESS;
