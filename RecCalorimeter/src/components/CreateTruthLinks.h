 ...
 *  - calorimeter clusters -> MC particles (weight = fraction of energy of given cluster due to a particular MC
 * particle)
 *  - MC particles -> calorimeter clusters (DOES NOT SEEM POSSIBLE IN EDM4HEP) => same relations as previous map but
 * different weight: w = fraction of total calo hit energy from given particle that goes in particular cluster Based on
 * https://github.com/iLCSoft/MarlinReco/blob/master/Analysis/RecoMCTruthLink/src/RecoMCTruthLinker.cc
 *
...
private:
  ...
  mutable k4FWCore::DataHandle<edm4hep::MCParticleCollection> m_mcparticles{"mcparticles", Gaudi::DataHandle::Reader,
                                                                            this};

  /// Handle for hit<->particle link (output collection)
  mutable k4FWCore::DataHandle<edm4hep::CaloHitMCParticleLinkCollection> m_cell_mcparticle_links{
      "cell_mcparticle_links", Gaudi::DataHandle::Writer, this};

  /// Handle for cluster<->particle link (output collection)
  mutable k4FWCore::DataHandle<edm4hep::ClusterMCParticleLinkCollection> m_cluster_mcparticle_links{
      "cluster_mcparticle_links", Gaudi::DataHandle::Writer, this};

  /// Handle for particle<->cluster link (output collection) [NOT POSSIBLE]
  //  mutable k4FWCore::DataHandle<edm4hep::MCParticleClusterLinkCollection>
  //  m_mcparticle_cluster_links{"mcparticle_cluster_links", Gaudi::DataHandle::Writer,
  //    this};
};
