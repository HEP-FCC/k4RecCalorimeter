...
CreateCaloClusters::CreateCaloClusters(const std::string& name, ISvcLocator* svcLoc) : Gaudi::Algorithm(name, svcLoc) {
  ...
  declareProperty("positionsECalTool", m_cellPositionsECalTool, "Handle for tool to retrieve cell positions in ECal");
  declareProperty("positionsHCalTool", m_cellPositionsHCalTool, "Handle for tool to retrieve cell positions in HCal");
...
StatusCode CreateCaloClusters::initialize() {
  ...
  m_totCalibEnergy =
      new TH1F("totalCalibEnergy", "total energy in all clusters after calobration per event", 5000, 0, 5);
  ...
  m_totBenchmarkEnergy =
      new TH1F("totBenchmarkEnergy",
               "total energy in all clusters after calobration and correction for lost energy in cryostat per event",
               5000, 0, 5);
  ...
  m_clusterEnergyBenchmark =
      new TH1F("clusterEnergyBenchmark", "energy of calibrated and energy loss corrected cluster", 20100, -100, 20000);
  ...
  m_nCluster_halfTrueEnergy =
      new TH1F("nCluster_halfTrueEnergy", "number of cluster with energy > Etrue/2", 20000, 0, 20000);
  if (m_histSvc->regHist("/rec/nCluster_halfTrueEnergy", m_nCluster_halfTrueEnergy).isFailure()) {
    error() << "Couldn't register hist" << endmsg;
    return StatusCode::FAILURE;
  }
  m_energyCalibCluster_1GeV =
      new TH1F("energyCalibCluster_1GeV", "energy of calibrated cluster with energy > 1GeV", 20100, -10, 20000);
  if (m_histSvc->regHist("/rec/energyCalibCluster_1GeV", m_energyCalibCluster_1GeV).isFailure()) {
    error() << "Couldn't register hist" << endmsg;
    return StatusCode::FAILURE;
  }
  m_energyCalibCluster_halfTrueEnergy = new TH1F(
      "energyCalibCluster_halfTrueEnergy", "energy of calibrated cluster with energy > Etrue/2", 20100, -100, 20000);
  ...
  m_energyScaleVsClusterEnergy = new TH2F(
      "energyScaleVsClusterEnergy", "energy scale of cluster versus energy of cluster", 3, 0., 3., 20000, 0, 20000);
...
StatusCode CreateCaloClusters::execute(const EventContext&) const {
  ...
  for (auto iparticle : *particles) {
    Etruth += sqrt(pow(iparticle.getMass(), 2) + pow(iparticle.getMomentum().x, 2) + pow(iparticle.getMomentum().y, 2) +
                   pow(iparticle.getMomentum().z, 2));
  }
  ...
  if (m_doCalibration) {
    for (auto cluster : *clusters) {
      ...
      if (static_cast<int>(cluster.getEnergy() * 100.0) !=
          static_cast<int>((energyBoth[m_systemIdECal] + energyBoth[m_systemIdHCal]) * 100.0))
        warning() << "The cluster energy is not equal to sum over cell energy: " << cluster.getEnergy() << ", "
                  << (energyBoth[m_systemIdECal] + energyBoth[m_systemIdHCal]) << endmsg;
      ...
      if (cellsInBoth) {
        ...
        if (m_doCryoCorrection) {
          debug() << "Energy in last ECal + first HCal layer: " << (energyLastECal + energyFirstHCal) << "GeV "
                  << endmsg;
          debug() << "Energy in last ECal x first HCal layer: " << (energyLastECal * energyFirstHCal) << "GeV "
                  << endmsg;
          double corr =
              m_b1 * sqrt(fabs(energyLastECal * m_a1 * energyFirstHCal)) + m_c1 * pow(lastBenchmarkTerm * m_a1, 2);
