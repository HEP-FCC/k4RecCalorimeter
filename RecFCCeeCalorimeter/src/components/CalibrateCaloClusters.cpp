...
StatusCode CalibrateCaloClusters::readCalibrationFile(const std::string &calibrationFile)
{
  ...
  {
    m_input_names.insert(m_input_names.end(), m_ortSession->GetInputName().begin(), m_ortSession->GetInputName().end());
  ...
  {
    m_output_names.insert(m_output_names.end(), m_ortSession->GetOutputName().begin(), m_ortSession->GetOutputName().end());
