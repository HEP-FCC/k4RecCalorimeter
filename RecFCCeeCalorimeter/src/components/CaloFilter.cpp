 ...
 * @date   2025-12-01
 *
 * Gaudi MultiTransformer that applies a filter to a TimeSeriesCollection of digitized pulses per cell, in order to
 *extract the output of this filter, the most likely energy, and the timing information for each cell as an index of the
 *pulse.
 *
 * This is a MultiTransformer that in v01 applies a matched filter to the digitized pulses. The matched filter is
 *defined using the time-reversed and conjugate of a known pulse shape (currently a Gaussian) of length \f$N\f$. This is
 *then convolved with the digitized pulse in order to extract the best estimate of the energy and timing information for
 *each cell. The matched filter provides the best signal to noise ratio in the presence of additive stochastic noise.
 *More information on the matched filter can be found at: https://en.wikipedia.org/wiki/Matched_filter and for this
 *specific application, see:
 *https://indico.cern.ch/event/1580025/contributions/6686602/attachments/3133485/5559196/FCCDigitization4BNLWorkshopEndOfWeekUpdate.pdf.
 *
 ...
 * \f$ \Sigma^{-1} \f$ is the inverse of the noise correlation matrix. Must be provided via a ROOT file during
 *initialization.
 * \f$ \vec{s} \f$ is the time reversed conjugate of the known signal shape. Currently assumed to be a Gausian shape
 *with a size < the number of samples in the digitized pulse.
 *
 * The output of the matched filter is then computed by convolving \f$ \vec{M} \f$ with the digitized pulse \f$ \vec{D}
 *\f$ as: \f$ \vec{O} = \vec{M} * \vec{D} \f$
 *
 * Note that the length of \f$ \vec{O} \f$ is set to be of the same size as \f$ \vec{D} \f$ in the convolution.
 * The maximum value of \f$ \vec{O} \f$ is taken as the best estimate of the energy in the cell, and the index of this
 *maximum value is taken as the best estimate of the timing.
 *
 ...
 *     - @param m_filterTemplateSize: The size of the known pulse shape to use for the matched filter. This should be a
 *number less than the number of samples in the digitized pulse. Odd number are preferred since even numbers can lead to
 *migration effects. This is typically (~4 or 5 in LAr).
 ...
 *     - @param m_invCorrMatName: The name of the ROOT TMatrixD that represents the inverse of the correlation matrix of
 *the noise.
 *
 * Outputs:
 ...
 *     - Only the matched filter, assuming a Gaussian signal pulse shape, is currently implemented. The OFC method might
 *be added in the future.
 *     - A nicer way to provide the signal shape would be good to have in the future, rather than hardcoding a Gaussian
 *shape.
 *     - The matched filter should technically be a conjugate but since we are dealing with real numbers only, this is
 *not done. Beware if complex pulse shapes are to be used in the future.
 */
...
struct CaloFilterFunc final
   ...
   *
   * This function computes the filter template (signal like part) based on the specified filter type (currently only
   * matched filter with Gaussian pulse shape is implemented) and the provided parameters. The inverse correlation
   * matrix is also loaded from a ROOT file for use in the filter computation.
   *
  ...
  StatusCode initialize() override {
    ...
    if (m_filterName.value() == "Matched_Gaussian") {
      ...
      // We will create a Gaussian shape w/ mean = m_mu and sigma = m_sigma. The filter will be defined as e^(-0.5 * ((x
   ...
   *
   * This function takes as input a collection of digitized pulses and applies the matched filter to each pulse. The
   * filtered pulses, along with the best estimate of the energy (max(filtered sample)) and timing (sample index of the
   * energy) for each pulse, are returned.
   *
   * \param DigitsPulse: The input collection of digitized pulses.
   * \return A tuple containing the filtered pulses, sample index representing the timing estimate, and the best
  ...
  StatusCode GetInvCorrMat() {
    ...
    if (NoiseInfoFile->IsZombie()) {
      ...
      info() << "Using the following file with noise info: " << m_noiseInfoFileName.value() << endmsg;
    }
   ...
   *
   * This function takes as input a digitized pulse and the matched filter template. It convolves the digitized pulse
   * with the time reversed version of the matched filter template and returns the filtered pulse.
   *
   ...
   *
   * This function computes \f$ G(x) = \frac{1}{\sigma \sqrt{2 \pi}} e^{-\frac{1}{2} \left( \frac{x - \mu}{\sigma}
   * \right)^2} \f$ for given x, mu, and sigma.
   *
   ...
   *
   * This function computes the derivative of the Gaussian \f$ G'(x) = -\left(\frac{x - \mu}{\sigma^2}\right) \cdot G(x)
   * \f$
   *
   * \param x: The input variable.
   * \param mu: The mean of the Gaussian.
   ...
   *
   * This function computes the convolution of the input pulse with the provided filter and returns a vector of the same
   * size as the input pulse. This is the equivalent of np.convolve(..., mode='same') in Python.
   * \param pulse: The input digitized pulse.
...
private:
  ...
  Gaudi::Property<std::string> m_noiseInfoFileName{this, "noiseInfoFileName", "NoiseInfo.root",
                                                   "Name of file to load noise corr matrix from"};
  Gaudi::Property<std::string> m_invCorrMatName{
      this, "invCorrMatName", "InvertedCorrelationMatrix",
      "Name of ROOT TMatrixD that represents the inverse of the correlation matrix of the noise"};
