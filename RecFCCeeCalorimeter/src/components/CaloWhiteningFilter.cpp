 ...
 *
 * The whitening filter is used to decorrelate noise between samples in the digitized pulses. The whitening matrix is
 *computed from the inverse of the noise correlation matrix, which is provided via a ROOT file during initialization.
 *The mean vector of the noise is also provided via the same ROOT file and is subtracted from each digitized pulse
 *before applying the whitening filter. The whitening operation is performed using the ZCA (Zero-phase Component
 *Analysis) method but there are other methods that can be implemented in the future.
 *
 * In mathematical terms, the ZCA method is:
 * \f$ \vec{W} = \Sigma^{-1/2} (\vec{D} - \vec{\mu})\f$
 * \f$ \vec{W} \f$ is a vector of the whitened pulse.
 * \f$ \Sigma^{-1/2} \f$ is the whitening matrix, defined as the inverse square root of the noise correlation matrix.
 *This should be precomputed and provided via the ROOT file.
 ...
 * Note that the vectors \f$ \vec{W} \f$, \f$ \vec{D} \f$, and \f$ \vec{\mu} \f$ have a length equal to the number of
 *samples in the digitized pulse and are thus defined per sample. More details for v01 be found at:
...
struct CaloWhitening final
   ...
   *
   * This function computes the stuff needed for the whitening filter (currently only ZCA whitening is implemented) and
   * the provided parameters. The inverse correlation matrix is also loaded from a ROOT file for use in the filter
   * computation.
   *
   ...
   *
   * This function takes as input a TimeSeriesCollection of digitized pulses per cell and applies the whitening filter
   * to each pulse. The resulting TimeSeriesCollection with whitened pulses is returned.
   *
   ...
   * \brief Applies the whitening filter to the digitized pulses.
   *
   * This function applies the whitening filter to a single digitized pulse by subtracting the mean vector and
   * multiplying by the whitening matrix.
   * \f$ \vec{W} = \Sigma^{-1/2} (\vec{D} - \vec{\mu})\f$
   *
...
private:
  ...
  Gaudi::Property<std::string> m_muVecName{this, "muVecName", "NoiseSampleMat_MeansVector",
                                           "Name of ROOT TVectorD that represents the mean vector of the noise"};
