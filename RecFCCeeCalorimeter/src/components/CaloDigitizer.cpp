 ...
 * @date   2025-12-01
 *
 * Gaudi Transformer that digitises a SimCalorimeterHitCollection from noble liquid calorimeter to
 *edm4hep::TimeSeriesCollection of digitized hits.
 *
 * The digitization uses a Gaussian pulse shape to simulate the response of the readout electronics. The mean and sigma
 *of the Gaussian pulse can be defined via properties. The digitization in v01 follows the LAr digitization as defined
 *in https://cds.cern.ch/record/1057879/files/larg-pub-2007-011.pdf?version=1. Note that currently, the pedestal is not
 *conisdered, noise is a work in progress, the ADC2MEV conversion is not considered, and the sampling fraction
 *correction is a work in progress. A more detailed explaination of the digitization procedure in v01, with the noise
 ...
 * Note that fundamentally, each hit belonging to the geometric area of a given cell is converted to a digitized pulse
 *in the corresponding cell, which are then summed to give the final digitized pulse in that cell.
 *
 ...
 *     - The digitization does not yet include the full noise simulation but a simpler one, in conjunction with
 *AddNoise2Digits.cpp.
 *     - The digitization does not yet include the sampling fraction correction, but this will be added in the future
 *via a separate algorithm.
...
struct CaloDigitizerFunc final
  ...
  CaloDigitizerFunc(const std::string& name, ISvcLocator* svcLoc)
      : Transformer(name, svcLoc, {KeyValues("InputCollection", {"SimCaloHitsCollection"})},
                    {KeyValues("OutputCollection", {"DigitsFloat"})}) {}

  /**
   * \brief Computes the pulse shape and its derivative.
   *
   * This function computes the pulse shape and its derivative based on the specified pulse type (currently only
   * Gaussian is implemented) and the provided mean and sigma values. The pulse shape and its derivative are stored in
   * member variables for later use during digitization.
   *
  ...
  StatusCode initialize() override {
    ...
    if (std::strcmp(m_pulseType.value().c_str(), "Gaussian") == 0) {
      ...
      for (int i = 0; i < m_lenSample.value(); ++i) {
        ...
        info() << "PulseShape[" << i * m_samplingInterval << "]: " << PulseShape[i] << ", PulseShapeDeriv["
               << i * m_samplingInterval << "]: " << PulseShapeDeriv[i] << endmsg;
      }
    } else {
   ...
   *
   * This function takes as input a SimCalorimeterHitCollection and digitizes each hit using the predefined pulse shape
   * and its derivative. The digitized pulses are stored in a TimeSeriesCollection, which is returned as output.
   *
  ...
  edm4hep::TimeSeriesCollection operator()(const edm4hep::SimCalorimeterHitCollection& CaloHits) const override {
    ...
    for (const auto& hit : CaloHits) {
      ...
      for (const auto& contribution : Contributions) {
        auto DigitVector =
            Hit2DigitFloat(contribution.getEnergy(), contribution.getTime(), PulseShape, PulseShapeDeriv);

        // Sum the contributions
        std::transform(DigitVectorSum.begin(), DigitVectorSum.end(), DigitVector.begin(), DigitVectorSum.begin(),
                       std::plus<float>());

        // Print the contribution info
        debug() << "Contribution (energy, time): (" << contribution.getEnergy() << ", " << contribution.getTime() << ")"
                << endmsg;
      }
   ...
   *
   * This function takes as input the energy and time of a hit, along with the pulse shape and its derivative, and
   * computes the corresponding digitized pulse samples. The digitized pulse is represented as a vector of floats. See
   * ATLAS LAr digitization note for details.
   *
   ...
   */
  std::vector<float> Hit2DigitFloat(float Energy, float time, const std::vector<float>& pulseShape,
                                    const std::vector<float>& pulseShapeDeriv) const {
