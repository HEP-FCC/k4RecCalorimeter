...
calo_init.parser.add_argument(
    "-t", "--title", default="Energy resolution", help="Graph title", type=str
)
calo_init.parser.add_argument(
    "-n",
    "--histogramName",
    default="energy",
    help="Name of the histogram containing the energy distribution",
    type=str,
)
calo_init.parser.add_argument("-m", "--axisMax", help="Maximum of the axis", type=float)
calo_init.parser.add_argument(
    "--noLinearity", help="If linearity plot should not be drawn", action="store_true"
)
calo_init.parser.add_argument(
    "--roundBrackets", help="Use round brackets for unit", action="store_true"
)
calo_init.parser.add_argument(
    "--technical", help="Print parameters with all details", action="store_true"
)
calo_init.parser.add_argument(
    "--specialLabel", help="Additional label to be plotted", type=str
)
...
from draw_functions import (
    ...
    prepare_single_canvas,
    prepare_double_canvas,
    draw_text,
)
import numpy
...
for energy, filename in [
    (20, "output_energy20Gev.root"),
    (10, "output_energy10Gev.root"),
    ...
    htotal = TH1F(
        "energy",
        "Energy of clusters (e^{-}, "
        + str(energy)
        + " GeV);energy (GeV);fraction of events",
        99,
        0.8 * energy,
        1.2 * energy,
    )
    dtree.Draw(
        "CaloClusters.core.energy>>energy",
        " sqrt(GenParticles.core.p4.px*GenParticles.core.p4.px + GenParticles.core.p4.py*GenParticles.core.p4.py + GenParticles.core.p4.pz*GenParticles.core.p4.pz) < "
        + str(energy + 0.1)
        + " && sqrt(GenParticles.core.p4.px*GenParticles.core.p4.px + GenParticles.core.p4.py*GenParticles.core.p4.p    y + GenParticles.core.p4.pz*GenParticles.core.p4.pz) > "
        + str(energy - 0.1),
    )
    C = TCanvas()
    htotal.Draw()
    ...
    myfunPre = TF1(
        "firstGaus",
        "gaus",
        htotal.GetMean() - 2.0 * htotal.GetRMS(),
        htotal.GetMean() + 2.0 * htotal.GetRMS(),
    )
    ...
    myfun = TF1(
        "finalGaus",
        "gaus",
        resultPre.Get().Parameter(1) - 2.0 * resultPre.Get().Parameter(2),
        resultPre.Get().Parameter(1) + 2.0 * resultPre.Get().Parameter(2),
    )
...
if calo_init.args.roundBrackets:
    prepare_graph(
        gRes, "resolution", ";E_{beam} (GeV);#sigma_{E_{rec}}/#LTE_{rec}#GT", colour, 21
    )
else:
    prepare_graph(
        gRes, "resolution", ";E_{beam} [GeV];#sigma_{E_{rec}}/#LTE_{rec}#GT", colour, 21
...
prepare_second_graph(
    gLin, gRes, "linearity", ";E_{beam} [GeV];(#LTE_{rec}#GT-E_{beam})/E_{beam}", factor
)
gLin.GetYaxis().SetRangeUser(-0.9, 0.9)

# Prepare canvas
if not calo_init.args.noLinearity:
    cRes, padRes, padLin = prepare_double_canvas(
        "resolution", "Energy resolution", factor
    )
    padRes.cd()
else:
...
formula = (
    "#frac{#sigma_{E}}{E} = "
    + str(round(fitResult.Get().Parameter(0) * 100, 2))
    + "% #oplus #frac{"
    + str(round(fitResult.Get().Parameter(2), 2))
    + "GeV}{E} #oplus #frac{"
    + str(round(fitResult.Get().Parameter(1) * 100, 2))
    + "%}{#sqrt{E}}"
)
if not calo_init.args.noLinearity:
...
if calo_init.args.technical:
    constString = (
        "const: "
        + str(round(fitResult.Get().Parameter(0), 4))
        + " #pm "
        + str(round(fitResult.Get().Error(0), 4))
    )
    samplingString = (
        "sampl: "
        + str(round(fitResult.Get().Parameter(1), 4))
        + " #pm "
        + str(round(fitResult.Get().Error(1), 4))
    )
    draw_text(
        [constString, samplingString], [0.55, 0.68, 0.88, 0.76], colour + 1, 0
    ).SetTextSize(0.05)
...
if calo_init.args.noLinearity and calo_init.args.specialLabel:
    draw_text(
        [calo_init.args.specialLabel], [0.67, 0.88, 0.95, 0.98], kGray + 3, 0
    ).SetTextSize(0.05)
cRes.Update()
if not calo_init.args.noLinearity:
    padLin.cd()
    ...
    if calo_init.args.specialLabel:
        draw_text(
            [calo_init.args.specialLabel], [0.67, 0.78, 0.95, 0.88], kGray + 3, 0
        ).SetTextSize(0.05 * factor)
