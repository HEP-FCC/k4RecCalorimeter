...
def draw_2histograms_normalized(
    histo1, histo2, x_axisName, y_axisName, leg1Name, leg2Name
):
    histo1.Scale(1.0 / histo1.GetEntries())
    histo2.Scale(1.0 / histo2.GetEntries())
...
def draw_text(lines, coordinates=[0.1, 0.8, 0.5, 0.9], colour=36, border=1):
    text = TPaveText(
        coordinates[0], coordinates[1], coordinates[2], coordinates[3], "brNDC"
    )
...
def prepare_double_canvas(name, title, factor=1):
    ...
    pad1 = TPad("pad1", "pad1", 0, 0, 1, factor / (1.0 + factor))
    pad2 = TPad("pad2", "pad2", 0, factor / (1.0 + factor), 1, 1)
    print("heights:", factor / (1.0 + factor))
...
def prepare_divided_canvas(name, title, N):
    ...
    print("=====> N : ", N)
    print(
        "=====> Dividing canvas into : ",
        int(ceil(sqrt(N))),
        int(ceil(N / ceil(sqrt(N)))),
    )
