...
def add_defaults():
    ...
    parser.add_argument(
        "energy", help="Energy of the particle [GeV]", type=int, nargs="+"
    )
    ## Optional arguments
    parser.add_argument(
        "-r",
        "--regex",
        help="String to insert in place of '?' character in file names (\"energy\" inserts the values of energies)",
        type=str,
        nargs="+",
    )
    ...
    if check:
        if len(regex) == 1 and regex[0] == "energy":
            ...
            if len(energies) > 1 and len(energies) != len(regex):
                print(
                    "If '-r/--regex' option is used, energy must be either an integer"
                )
